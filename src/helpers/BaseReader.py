...
class BaseReader(object):
    ...
    def _load_preprocessed_df(self):
        ...
        # delete query related triple
        query_flag_list = []
        for head, relation, tail in zip(self.relation_df['head'].tolist(), self.relation_df['relation'].tolist(), self.relation_df['tail'].tolist()):
            if (head >= min_query and head < max_query) or (tail >= min_query and tail < max_query):
                query_flag_list.append(1)
            else:
                query_flag_list.append(0)
        self.relation_df["query_flag"] = query_flag_list
        self.relation_df = self.relation_df[self.relation_df["query_flag"] == 0]
        ...
        train_user, train_item, history_items = [], [], []
        for a, b, c, _ in data_train:
        ...
        def get_test_data(file_name):
            ...
            test_user, test_item, test_neg_item, history_items = [], [], [], []
